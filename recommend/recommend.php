<?php
	class Recommend{
        ...
        function GetTags($UserOrArticles){
			...
			switch ($From) {
            	case 'Users':
					...
					if (!empty($tagsToUse))
                    {
                        ...
                        {	
                        	while ($row = $result->fetch_assoc()){
                                $articlesByUsers = $articlesByUsers . '<div class="col s12 m3">
        		...
        		case 'Articles':
					...
					if (!empty($tagsToUse))
                    {
                        ...
                        {
                        	while ($row = $result->fetch_assoc()){
                                $articlesByArticles = $articlesByArticles . '<div class="col s12 m3">
                                    <div class="card hoverable item" id="'.$row['PostID'].'">
                                        <div class="card-image">
                                            <img src="https://i.ytimg.com/vi/2OgLKz9yQ0Q/hqdefault.jpg?custom=true&w=246&h=138&stc=true&jpg444=true&jpgq=90&sp=68&sigh=IE1JFDEOZl_4r872Wlo5ydYUKjc">
                                        </div>
